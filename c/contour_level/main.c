...
int Surface_Contour(gsl_matrix *d,int ilb,int iub,int jlb,int jub,
			 double *x,double *y,int nc,double *z, double val , gsl_matrix *out ,
			 char *namefile , bool write_in_file )
{
#define xsect(p1,p2) (h[p2]*xh[p1]-h[p1]*xh[p2])/(h[p2]-h[p1])
#define ysect(p1,p2) (h[p2]*yh[p1]-h[p1]*yh[p2])/(h[p2]-h[p1])

// cherche les 'nc' lignes de niveaux au hauteur données dans 'z' de la matrice d.
// renvoie la matrice out contenant les points (x_i , y_i) formant le contour
// à la hauteur 'val'.
// si write_in_file est vrai alors tout est ecrit dans le fichier dont le 
// nom et 'namefile'
			...
			*/
			...
			for (k=0;k<nc;k++) 
			{
				...
				for (m=4;m>=0;m--) 
				{
					if (m > 0) 
					{
						temp_diimmm1 = gsl_matrix_get ( d , (size_t) i+im[m-1] , (size_t) j+jm[m-1] );
				...
				for (m=1;m<=4;m++) 
				{
					...
					if ( write_in_file )
					{
						fprintf ( file , _DFMT_" "_DFMT_" "_DFMT_" "_DFMT_"\n",x1,y1,x2,y2,z[k] );
					}
...
int main ( void )
{
	...
	int nb_mat = 
		Surface_Contour(m,0,x_nbpts-1,0,y_nbpts-1,x,y,contour_nb,contour_tab,0.
						,m_out,"test_write.dat",true);
