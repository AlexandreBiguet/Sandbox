...
/* Essai d'un bloc.                                                      */
#define try if (!((++SLOGL_catch.current)->type = \
                      setjmp(SLOGL_catch.current->env)))

/* Bloc de traitement d'erreur.                                          */
#define catch (id) if (!SLOGL_catch.current->type)-- SLOGL_catch.current; \
    else for (int SLOGL_tmp = 1, id = (SLOGL_catch.current--)->type;      \
              SLOGL_tmp; SLOGL_tmp = 0)

/* Signalement d'une erreur.                                             */
#define throw(t) longjmp(SLOGL_catch.current->env, t)
...
static void printLevel(SLOGL_level msgLevel) {
  if (fpSave && msgLevel <= SLOGL_LVL_FATAL) {
    static const char *t[] = {"Debug",   "Info",  "Notice",
...
static void printFunction(SLOGL_frame *this, int started) {
  ...
  SLOGL_vprint(SLOGL_LVL_DEBUG, "%s %s[%p]", started ? "END" : "START",
               this->name, this->addr);
}

/* Récupère une description de l'erreur.                                 */
char *SLOGL_printDesError(SLOGL_err n) {
  static const char *t[] = {"aucune erreur",          "allocation mémoire",
                            "ouverture d'un fichier", "fermeture d'un fichier",
                            "paramètre invalide",     "division par zéro"};
...
/* Positionne l'affichage ou non de la pile d'appel au fur et à mesure.  */
void SLOGL_setStackTrace(int n) { printStackTrace = n; }

/* Positionne le niveau d'erreur du programme.                           */
void SLOGL_setProgramLevel(SLOGL_level lvl) { SLOGL_programLevel = lvl; }
...
void SLOGL_pushStack(char *name, void *addr, char *file, int line) {
  ...
  if (this) {
    ...
    for (int i = 0; i < SLOGL_MAXCALLFUNCTION; ++i) this->up[i] = NULL;

    if (!currentStack.head) {
      if (printStackTrace) SLOGL_printHeader(SLOGL_LVL_DEBUG);
      currentStack.head = this;
    }

    if (tmp) tmp->up[tmp->iMax++] = this;

    currentStack.current = this;

    if (printStackTrace) printFunction(currentStack.current, 0);
  }
}
...
void SLOGL_popStack(int endLine) {
  if (currentStack.current) {
    ...
    if (printStackTrace) printFunction(currentStack.current, 1);
...
void SLOGL_displayStack(void) {
  ...
  while (tmp) {
    if (tmp->up[tmp->iUp]) {
      ...
      if (tmp->endLine) printFunction(tmp, 1);
  ...
  if (currentStack.head->endLine) printFunction(tmp, 1);
...
int SLOGL_init(const char *fileName) {
  ...
  if (fileName) {
    ...
    snprintf(fullFileName, sizeof fullFileName, "%s.%d%d%d.log", fileName,
...
void foo(unsigned val) START(foo);
if (val) {
  foo(val / 2);
  putchar((val % 2) ? '1' : '0');
}
...
void test() START(test);
printf("bla\n");
END();

void test2() START(test2);
test();
END();

void test3() START(test3);
test2();
END();

void test4() START(test4);
test3();
END();
