...
void TEST_GSLran() {
  /*----------------------------------------------------------------------
          ...
          PART 1 : (part1 = true)
          ```````````````````````
                          Simple test for the 4 previous routine, one random
  number is generated between min and max for each RNG.
          ...
          PART 3 : (part3 = true)
          ```````````````````````
                          Test of convergence true the theoretical mean for the
  4 RNG routines. PART 4 : (part4 = true)
          ```````````````````````
...
void TEST_gaus_dist() {
  ...
  double gd_aux(double x) { return GSLgauss_pdf(x, sigma); }
...
void TEST_gaus_tail_dist() {
  ...
  double gt_aux(double x) { return GSLgauss_tail_pdf(x, lomin, sigma); }
  sprintf(namedir, "mkdir -p data/test/GSL-Random-Nb-Gen/pdf");
  system(namedir);
  sprintf(c1, "data/test/GSL-Random-Nb-Gen/pdf/gaus_tail_pdf");
  sprintf(c2, "pdf gauss-tail {/Symbol s} = %.2f lower bound = %.2f", sigma,
          lomin);
...
void TEST_biv_gaus_dist() { printf("\n Bivariate Gaussian Distribution \n "); }
...
void TEST_exp_dist() {
  ...
  double faux(double x) { return GSLexp_pdf(x, mean); }
  sprintf(namedir, "mkdir -p data/test/GSL-Random-Nb-Gen/pdf");
  system(namedir);
...
void TEST_laplace_dist() {
  ...
  double faux(double x) { return GSLlaplace_pdf(x, width); }
  sprintf(namedir, "mkdir -p data/test/GSL-Random-Nb-Gen/pdf");
  system(namedir);
