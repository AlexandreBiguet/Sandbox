...
======================================================================*/
//
void makeHisto(double *tab, int tsize, double *histo, int nbin, double xmin,
               double xmax) {
  /*----------------------------------------------------------------------
...
void PlotCFct(double f(double), double xmin, double xmax, int n, char *nameFile,
              char *nameGraph, char *nameXaxis, char *nameYaxis,
              char *command) {
  char Dname[1024];
  char Gname[1024];
...
void Plot3DCFct(double f(double, double), double xmin, double xmax, double ymin,
                ...
                char *nameXaxis, char *nameYaxis, char *nameZaxis,
                char *command, bool keepdata) {
  double itx, ity;
  ...
  for (int i = 0; i <= nx; i++) {
    for (int j = 0; j <= ny; j++) {
      fprintf(fi, "%15.7f %15.7f %15.7f \n", xmin + i * itx, ymin + j * ity,
              f(xmin + i * itx, ymin + j * ity));
    }
    fprintf(fi, "\n");
  }
  fclose(fi);
  ...
  if (!keepdata) {
    sprintf(comm,
            "gnuplot %s ; epstopdf %s ; epstopdf %s_1.eps ;"
