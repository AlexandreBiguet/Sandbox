...
int integ_qags(double f(double), double bmin, double bmax, double *res,
               double *err);
int integ_qagiu(double f(double), double bmin, double *res, double *err);
int integ_d3p(double f(double), double bmax, double *res, double *err,
              bool cutoff);
...
int r1d_brent_method(double f(double), double xlo, double xhi, double *zer,
                     double *err);
int resMFE_m(parNJL *njl, double T, double C, double mlo, double mhi,
             double *res);
...
int integ_qags(double f(double), double bmin, double bmax, double *res,
  ...
  F.function = &faux;
  int status =
      gsl_integration_qags(&F, bmin, bmax, gsl.ea, gsl.er, gsl.ws, w, res, err);
...
int integ_qagiu(double f(double), double bmin, double *res, double *err) {
  ...
  F.function = &faux;
  int status =
      gsl_integration_qagiu(&F, bmin, gsl.ea, gsl.er, gsl.ws, w, res, err);
  gsl_integration_workspace_free(w);
  gsl_set_error_handler(old_handler);
  return status;
}
...
void test_integration(void) {
  ...
  for (size_t i = 0; i <= N; i++) {
    ...
    fprintf(f, "%20.15f %20.15f %20.15f %20.15f %20.15f %20.15f \n", p, p2,
            fd(p2), int_cut(p), int_inf(p2), integrand(p));
  }
  fclose(f);
  ...
  double Iin(double p, double t) {
    return -1.0 * int_cut(p) * (fdt(p, t) + fbt(p, t));
  }
  ...
  for (size_t i = 0; i < NT; i++) {
    ...
    fprintf(f,
            "%30.15f %30.15f %30.15f %30.15f %30.15f %30.15f %30.15f %15s %15s "
            "%15s\n",
            T, resint, resint_inf, resint_cut, errint, errint_inf, errint_cut,
...
int PV_I1_a(double L, double m, double *I) {
  *I = 1. / (8. * M_PI * M_PI) *
       (L * EP_C - m * m * log(EP_C + L) + m * m * log(m));
...
void test_precision(void) {
  ...
  fprintf(f,
          ...
          "# itC  = %20.15f	\n",
          njl.L, njl.L * njl.L * njl.G, njl.m0, Tmin, Tmax, Cmin, Cmax, Mmin,
          Mmax, NT, NC, itT, itC);
