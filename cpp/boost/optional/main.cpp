...
struct Object2 {
  ...
  /// \param one
  /// \param two
  explicit Object2(const T& one, const T& two) : _one(one), _two(two) {}

  /// Print
  void print() const { std::cout << _one << "  " << _two << std::endl; }
...
struct ObjectHolder {
  ...
  /// Getter
  /// \return
  const Obj<T>& object() const { return _object.get(); }
  ...
  /// Print
  void print() const { _object->print(); }
