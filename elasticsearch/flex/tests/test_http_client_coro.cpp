...
int main(int argc, char **argv) {
  ...
  if (argc != 4 && argc != 5) {
    std::cerr << "Usage: test_http_client_coro <host> <port> <target> [<HTTP "
                 "version: 1.0 or 1.1(default)>]\n"
              << "Example:\n"
              << "    test_http_client_coro www.example.com 80 /\n"
              << "    test_http_client_coro www.example.com 80 / 1.0\n";
    return EXIT_FAILURE;
  }
  ...
  boost::asio::spawn(ioc,
                     std::bind(&do_session, std::string(host),
                               std::string(port), std::string(target), version,
